...
// gedir — region/kateqoriya linkləri CustomerPortal-ın oxuduğu /?region= və /?category=
// query param-larına, tur linkləri /tours/:slug səhifələrinə.
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-[1.4fr_1fr_1.3fr_1fr] gap-8">
          ...
          <nav aria-label={t('customerMisc.siteFooter.popularDestinations')}>
            ...
            <ul className="space-y-2">
              {destinations.map(([name, count]) => (
                <li key={name}>
                  <Link to={`/?region=${encodeURIComponent(name)}`} className={`${linkClass} inline-flex items-center gap-1.5`}>
          ...
          <nav aria-label={t('customerMisc.siteFooter.popularTours')}>
            ...
            <ul className="space-y-2">
              {popularTours.map(tour => (
                <li key={tour.id}>
                  <Link to={`/tours/${tour.slug || tour.id}`} className={`${linkClass} block truncate max-w-[240px]`}>
          ...
          <div className="space-y-8">
            <nav aria-label={t('customerMisc.siteFooter.categoriesHeading')}>
              ...
              <ul className="space-y-2">
                <li>
                  <Link to="/" className={linkClass}>{t('customerMisc.siteFooter.allTours')}</Link>
                </li>
                {presentCategories.map(cat => (
                  <li key={cat}>
                    <Link to={`/?category=${cat}`} className={linkClass}>
