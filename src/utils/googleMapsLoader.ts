 ...
 *
 * The API key is baked in at build time via vite.config.ts's `define` (GOOGLE_MAPS_PLATFORM_KEY
 * env var) — it is a browser/Maps-JS key, meant to be used client-side and restricted by HTTP
 * referrer in the Google Cloud Console, not a server secret.
 */
...
export function isGoogleMapsConfigured(): boolean {
  const key = (typeof process !== 'undefined' ? process.env.GOOGLE_MAPS_PLATFORM_KEY : '') || '';
