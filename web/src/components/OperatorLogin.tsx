...
export default function OperatorLogin({ onLogin }: OperatorLoginProps) {
  ...
  // modal — mirrors AdminLogin's twin implementation.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t('miscWidgets.operatorLogin.connectionError'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        setForgotError(data.error || t('miscWidgets.operatorLogin.connectionError'));
        return;
      }
      setForgotMessage(t('miscWidgets.forgotPassword.successMessage'));
    } catch (e: any) {
      setForgotError(t('miscWidgets.operatorLogin.connectionError'));
    } finally {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white max-w-sm w-full rounded-2xl shadow-xl overflow-hidden border border-slate-100">
        <div className="bg-slate-900 p-6 text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-1">{t('miscWidgets.operatorLogin.title')}</h2>
          <p className="text-xs text-slate-400">
            {showForgotPassword ? t('miscWidgets.forgotPassword.subtitle') : t('miscWidgets.operatorLogin.subtitle')}
          </p>
        </div>
        <div className="p-6">
          {showForgotPassword ? (
              ...
              {forgotMessage ? (
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5">
                    ...
                    {t('miscWidgets.operatorLogin.usernameLabel')}
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder={t('miscWidgets.operatorLogin.usernamePlaceholder')}
                    required
                  />
                </div>
              )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5">
                  ...
                  {t('miscWidgets.operatorLogin.passwordLabel')}
                </label>
                <input
                  type="password"
                  value={password}
              ...
              <button
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl shadow-sm text-sm transition-all disabled:opacity-70 flex justify-center mt-2"
              >
                {isLoading ? t('miscWidgets.operatorLogin.loggingIn') : t('miscWidgets.operatorLogin.login')}
              </button>
              <button
                type="button"
